"""절기(節) 테이블 생성 → bazi/_data/jieqi_1800_2200_min.npz

12절(節): 小寒·立春·驚蟄·清明·立夏·芒種·小暑·立秋·白露·寒露·立冬·大雪
사주 월주 계산 기준. 절기 시각(분 단위, Unix epoch 기준) + 월 인덱스 + 태양력 연도 저장.
...
OUT = Path(__file__).parent.parent / "bazi/_data/jieqi_1800_2200_min.npz"
...
def build():
    ...
    for year in range(START_YEAR - 1, END_YEAR):
        ...
        for raw_i, jd in enumerate(jd_list):
            ...
            minutes = round((jd - _JD_UNIX) * 24 * 60)
            ...
            entries.append((minutes, month_idx, year))

    entries.sort(key=lambda x: x[0])

    min_arr   = np.array([e[0] for e in entries], dtype=np.int64)
    month_arr = np.array([e[1] for e in entries], dtype=np.int8)
    year_arr  = np.array([e[2] for e in entries], dtype=np.int16)

    np.savez_compressed(OUT, min_arr=min_arr, month_idx=month_arr, year_arr=year_arr)
    print(f"저장 완료: {OUT}")
    print(f"  절기 개수: {len(min_arr)}")
    diffs = np.diff(min_arr)
    print(f"  절기 간격: {diffs.min()/1440:.1f}~{diffs.max()/1440:.1f}일 (평균 {diffs.mean()/1440:.2f}일)")
