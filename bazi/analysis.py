...
def _ss_name(idx: int) -> str:
    return _SHI_SHEN_NAMES_BY_LANG[get_lang()][idx]
...
School = Literal["traditional", "ziping"]

_SCHOOL_HIDDEN_STEMS: dict[str, list[list[int]]] = {
    "traditional": BRANCH_HIDDEN_STEMS,
    "ziping": DAYUN_HIDDEN_STEMS,
}
...
def analyze(
    ...
    *,
    ...
    Args:
        ...
        school: 학파 ('traditional' | 'ziping')
        sex: 성별 (대운 계산에 사용)
        birth: 생년월일 (대운 계산 시 필요)
        dayun_count: 뽑을 대운 개수 (기본 8개)

    출력 언어는 bazi.set_lang() 또는 환경변수 BAZI_LANG ('zh'|'ko'|'en', 기본 'zh')
    """
    hidden_stems_table = _SCHOOL_HIDDEN_STEMS[school]
    ...
    for key, p in pillars_raw.items():
        ...
        hidden = [STEMS[i] for i in hidden_stems_table[b_idx]]
        main_hidden_idx = STEMS.index(hidden[-1])

        if key == "day":
            stem_ss = _DAY_LABEL[get_lang()]
            branch_ss = _ss_name(_shi_shen_idx(day_stem_idx, main_hidden_idx))
        else:
            stem_ss = _ss_name(_shi_shen_idx(day_stem_idx, s_idx))
            branch_ss = _ss_name(_shi_shen_idx(day_stem_idx, main_hidden_idx))
    ...
    if birth is not None:
        dayun_list = _calc_dayun(chart, birth, sex, day_stem_idx, dayun_count)
...
def _calc_dayun(
    ...
    for i in range(1, count + 1):
        ...
        dayuns.append(DaYun(
            ...
            stem_element=STEM_ELEMENT[s_idx],
            branch_element=BRANCH_ELEMENT[b_idx],
            stem_shi_shen=_ss_name(_shi_shen_idx(day_stem_idx, s_idx)),
            branch_shi_shen=_ss_name(_shi_shen_idx(day_stem_idx, main_hidden_idx)),
        ))
    return dayuns
